...
const SimulationScreen = () => {
  ...
  const [dependentes, setDependentes] = useState([{ nome: '', idade: '', parentesco: '' }]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post('http://seu-backend.com/salvar-dados', {
        nome,
    ...
    <div className="simulation-container">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="form-group">
          ...
          <select id="cidade" value={cidadeSelecionada} onChange={(e) => setCidadeSelecionada(e.target.value)}>
        ...
        {temPlano && (
          <div className="form-group">
            <label>
              Qual plano?
              <input type="text" value={planoSelecionado} onChange={(e) => setPlanoSelecionado(e.target.value)} />
            </label>
          </div>
        )}
        <div className="form-group">
          <label htmlFor="idade">Faixa etária:</label>
          <select id="idade" value={idadeSelecionada} onChange={(e) => setIdadeSelecionada(e.target.value)}>
            <option value="">Selecione...</option>
            {idadeOpcoes.map((opcao) => (
              <option key={opcao.id} value={opcao.valor}>{opcao.label}</option>
            ))}
          </select>
        </div>
        <div className="form-group">
          <label htmlFor="plano">Plano desejado:</label>
          <select id="plano" value={planoSelecionado} onChange={(e) => setPlanoSelecionado(e.target.value)}>
            <option value="">Selecione...</option>
            {planos.map((plano) => (
        ...
        <div className="form-group">
          ...
          <select id="seguro" value={seguroSelecionado} onChange={(e) => setSeguroSelecionado(e.target.value)}>
            <option value="">Selecione...</option>
        ...
        <div className="form-group toggle-label">
          ...
          <div className="toggle-button" onClick={() => setDesejaDependente(!desejaDependente)}>
            <div className={`slider ${desejaDependente ? 'checked' : ''}`}></div>
          </div>
        </div>
          ...
          <div>
            {dependentes.map((dependente, index) => (
              <div key={index} className="form-group dependent-card">
                ...
                <button type="button" onClick={() => handleRemoveDependente(index)}>
                  Remover
                </button>
              </div>
            ))}
            <button type="button" onClick={handleAddDependente} className="form-group">
